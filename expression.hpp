...
struct Expression{
  ...
  //return pointer to the last expression in the tail
  Expression * tailPtr();
  ...
  Expression handle_lookup(const Atom & headAtom, const Environment & env);
  Expression handle_define(Environment & env, std::vector<Atom> &graphical);
  Expression handle_begin(Environment & env, std::vector<Atom> &graphical);
  Expression handle_draw(Environment & env, std::vector<Atom> &graphical);

  Expression evaluate(Environment & env, std::vector<Atom> &graphical);
};
...
//check if token is numeric value or not
bool is_numeric(std::string token);

//comparison of two atoms
bool operator== (const Atom &a, const Atom &b);

//anti comparison of two expressions
